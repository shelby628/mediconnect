...
const EMPTY = { full_name: '', department: 'Cardiology', specialty: '', status: 'Available' };
...
const ManageDoctors = () => {
    ...
    const getDisplayName = (doc) => doc.full_name || doc.name || 'Unknown';
                            ...
                            <motion.tr layout key={doc.id} style={{ borderBottom: i < doctors.length - 1 ? '1px solid #F2EDE8' : 'none' }}>
                                <td style={{ padding: '16px 20px' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                        ...
                                        <span style={{ fontWeight: 700 }}>{getDisplayName(doc)}</span>
                                    </div>
                                </td>
                    ...
                    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 32 }}>
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} style={{ background: '#fff', borderRadius: 28, padding: 40, width: '100%', maxWidth: 480, position: 'relative', boxShadow: '0 24px 60px rgba(0,0,0,0.15)' }}>
                            ...
                            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                                {/* Full Name */}
                                <div>
                                    <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#6B6460', marginBottom: 6 }}>Full Name</label>
                                    <input
                                        required
                                        type="text"
                                ...
                                {/* Specialty */}
                                <div>
                                    <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#6B6460', marginBottom: 6 }}>Specialty</label>
                                    <input
                                        required
                                        type="text"
                ...
                {confirmDelete && (
                    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 32 }}>
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} style={{ background: '#fff', borderRadius: 24, padding: 36, width: '100%', maxWidth: 400, position: 'relative', textAlign: 'center', boxShadow: '0 24px 60px rgba(0,0,0,0.15)' }}>
                            ...
                            <p style={{ color: '#6B6460', fontSize: '0.9rem', marginBottom: 28 }}>
                                Are you sure you want to remove <strong>{getDisplayName(confirmDelete)}</strong>? This cannot be undone.
                            </p>
